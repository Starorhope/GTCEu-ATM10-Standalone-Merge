...
public class ConditionalRecipe {
    ...
    public static class Builder {
        ...
        public void save(RecipeOutput out, ResourceLocation id) {
            ...
            for (var pair : recipes) {
                if (pair.recipe().m_126373_() != null) {
                    adv.condition(pair.condition())
                       .advancement(pair.recipe().m_126373_());
                    hasAdvancement = true;
                }
            }
            ...
            out.m_292927_(new Finished(id, mainCondition, recipes, advancementId, hasAdvancement ? adv.write() : null));
        }
    }
    ...
    private record Finished(
        ...
        public void m_7917_(JsonObject json) {
            ...
            for (var pair : recipes) {
                var holder = pair.recipe().m_125966_();
    ...
    private static Codec<Recipe<?>> CODEC = new Codec<Recipe<?>>() {
        ...
        public <T> DataResult<Pair<Recipe<?>, T>> decode(DynamicOps<T> ops, T input) {
            ...
            try {
                var recipes = GsonHelper.m_13933_(GsonHelper.m_13918_(json.getAsJsonObject(), "root"), "recipes");
                int idx = 0;
                for (var entry : recipes) {
                    var object = GsonHelper.m_13918_(entry, "recipe[" + idx++ + "]");
                    var conditionjson = GsonHelper.m_13930_(object, ICondition.DEFAULT_FIELD);
                    var condition = Util.m_260975_(ICondition.SAFE_CODEC.parse(JsonOps.INSTANCE, conditionjson), JsonSyntaxException::new);
                    if (!condition.test(context))
                        continue;
                    var type = new ResourceLocation(GsonHelper.m_13906_(object, "type"));
                    ...
                    var parsed = serializer.m_292673_().parse(JsonOps.INSTANCE, object);
