...
public class LargeSteamExtractor extends SteamMultiMachineBase {


    @SaveField
    private int targetParallel = 24;

    public LargeSteamExtractor(BlockEntityCreationInfo holder, Object... args) {
        super(holder, false, args);
    }

    @Nullable
    public static ModifierFunction recipeModifier(MetaMachine machine, @Nonnull GTRecipe recipe) {
        if (!(machine instanceof LargeSteamExtractor steamMachine)) {
            return ModifierFunction.NULL;
        }
        if (recipe.getType() != GTRecipeTypes.EXTRACTOR_RECIPES) {
            return ModifierFunction.NULL;
        }
    ...
    public void handleDisplayClick(String componentData, ClickData clickData) {
        if (!clickData.isRemote) {
            if (componentData.equals("parallelSub")) {
                ...
                this.targetParallel = Math.min(48, this.targetParallel * 2);
            }
        }
    }
}
