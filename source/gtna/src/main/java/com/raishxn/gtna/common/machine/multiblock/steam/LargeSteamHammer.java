...
public class LargeSteamHammer extends SteamMultiMachineBase {


    @SaveField
    private int targetParallel = 32;

    public LargeSteamHammer(BlockEntityCreationInfo holder, Object... args) {
        super(holder, false, args);
    }

    @Nullable
    public static ModifierFunction recipeModifier(MetaMachine machine, @Nonnull GTRecipe recipe) {
        if (!(machine instanceof LargeSteamHammer steamMachine)) {
            return ModifierFunction.NULL;
        }
        if (recipe.getType() != GTRecipeTypes.FORGE_HAMMER_RECIPES) {
            return ModifierFunction.NULL;
        }
