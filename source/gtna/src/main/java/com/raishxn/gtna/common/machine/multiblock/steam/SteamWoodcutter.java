...
public class SteamWoodcutter extends SteamMultiMachineBase {


    @SaveField
    private int targetParallel = 64;

    public SteamWoodcutter(BlockEntityCreationInfo holder, Object... args) {
        super(holder, false, args);
    }

    @Nullable
    public static ModifierFunction recipeModifier(MetaMachine machine, @Nonnull GTRecipe recipe) {
        if (!(machine instanceof SteamWoodcutter steamMachine)) {
            return ModifierFunction.NULL;
        }
        if (recipe.getType() != GTNARecipeType.WOODCUTTER_RECIPES) {
            return ModifierFunction.NULL;
        }
